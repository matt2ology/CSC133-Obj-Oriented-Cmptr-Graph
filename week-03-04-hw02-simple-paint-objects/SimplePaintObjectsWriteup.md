<!-- Use Obsidian notes to render Markdown to PDF -->
# Simple Paint Objects Write-up

- [Simple Paint Objects Write-up](#simple-paint-objects-write-up)
  - [Write-up Questions](#write-up-questions)
    - [1) How did the additional code requirements hinder your solution to this problem?](#1-how-did-the-additional-code-requirements-hinder-your-solution-to-this-problem)
    - [2) What are some advantages to these code requirements going forward?](#2-what-are-some-advantages-to-these-code-requirements-going-forward)
    - [3) Where do you think the required class hierarchy gets in the way of code reuse?](#3-where-do-you-think-the-required-class-hierarchy-gets-in-the-way-of-code-reuse)
      - [Example: "Icons can be made generically"](#example-icons-can-be-made-generically)
...
No additional code requirements hindered my solution to this problem; for, a
specified and well defined list of requirements are easy to target code against
and behaviors that are outside of specification are considered "bugs".

The only issue I have with the homework prompt is that an approach to how one
may approach the problem would be offered, but later abandoned in favor for an
optimal, better, solution: a great tactic to lead students a stray if they
read section by section coding to the requirements and to later find out what
...
As answered in question prompt one, having code requirements defines the
acceptable state, it defines a scope of work where behaviors outside of what
has been prescribed in the requirements documentation are "bugs", and (if well
written) little should be left up to interpretation.

### 3) Where do you think the required class hierarchy gets in the way of code reuse?

> (To think about this problem, where did you find the hierarchy tedious in
> terms of typing code that is almost the same from one class to the next,
> e.g., the classes derived from FilledPolyShape)

As it stands now... With the current required class hierarchy that gets in the
way of code reuse is by way of not having a generic class that will create the
icons of each of the drawing tools and action (PointTool, LineTool,
RectangleTool OvalTool, RoundedRectangleTool, and the Action "ClearTool").
